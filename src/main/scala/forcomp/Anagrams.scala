...
object Anagrams extends AnagramsInterface {
   ...
   *
   */
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = dictionary map (word => (word, wordOccurrences(word))) groupBy(_._2) map (kv => (kv._1, kv._2.map({case(w,l) => w})))
